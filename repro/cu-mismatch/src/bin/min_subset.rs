...
fn main() -> Result<()> {
    ...
    println!(
        "baseline: rpc_units={} rpc_err={} litesvm_cu={} litesvm_err={} delta={}",
        opt_u64_to_string(baseline.rpc_units),
        baseline
            .rpc_err
            .clone()
        ...
        baseline
            ...
            .clone()
            .unwrap_or_else(|| "null".to_string()),
        opt_i128_to_string(baseline.delta())
    );

    let n = instruction_count(&original_tx);
    println!("top_level_instruction_count={n}");

    let mut found_any = false;
    for k in 1..=n {
        let combos = combinations(n, k);
        let mut matches: Vec<(Vec<usize>, EvalOut)> = Vec::new();
        for keep in combos {
            let tx = keep_only_instructions(&original_tx, &keep)?;
            ...
            if is_cu_mismatch(&eval, threshold) {
                matches.push((keep, eval));
            }
        }

        if !matches.is_empty() {
            found_any = true;
            println!("minimal_keep_size={k}");
            for (keep, eval) in matches {
                println!(
                    "keep={keep:?} rpc_units={} litesvm_cu={} delta={}",
                    opt_u64_to_string(eval.rpc_units),
                    eval.litesvm_cu,
                    opt_i128_to_string(eval.delta())
                );
            }
            break;
        }
    ...
    if !found_any {
        println!("no subset found where both simulations succeed and CU delta >= threshold");
    }
...
fn rpc_simulate(rpc_url: &str, tx: &VersionedTransaction, curl_timeout: u64) -> Result<RpcSimOut> {
    ...
    let mut output_bytes: Option<Vec<u8>> = None;
    for attempt in 1..=retries {
        ...
        if output.status.success() {
            output_bytes = Some(output.stdout);
            break;
        }
    ...
    let Some(stdout) = output_bytes else {
        bail!(
    ...
    let v: Value = serde_json::from_slice(&stdout).context("parse simulateTransaction JSON")?;
    let rpc_error = v
...
fn opt_i128_to_string(v: Option<i128>) -> String {
    v.map(|n| n.to_string())
        .unwrap_or_else(|| "null".to_string())
}
...
impl EvalOut {
    fn delta(&self) -> Option<i128> {
        self.rpc_units.map(|u| self.litesvm_cu as i128 - u as i128)
    }
}
