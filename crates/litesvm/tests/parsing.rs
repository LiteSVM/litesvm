...
#[test]
fn test_inner_instruction_parsing_rent_exemption_deactivated() {
    let mut feature_set = FeatureSet::all_enabled();
    feature_set.deactivate(&agave_feature_set::deprecate_rent_exemption_threshold::id());
    let mut svm = LiteSVM::new().with_feature_set(feature_set).with_sysvars();
...
fn test_inner_instruction_parsing() {
    ...
    assert_eq!(
        vec![2, 0, 0, 0, 64, 197, 9, 0, 0, 0, 0, 0],
        result.inner_instructions[0][0].instruction.data
    );
}
