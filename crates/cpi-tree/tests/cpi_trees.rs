...
// The fixtures live in the litesvm crate's `test_programs` (built by CI via
// `cargo build-sbf`); we reach across to the already-built .so files rather
// than duplicate the fixture programs here.
...
fn read_program(file: &str) -> Vec<u8> {
    ...
    p.push(format!("../litesvm/test_programs/target/deploy/{file}"));
